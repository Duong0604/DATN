...
class AdminQuanlyController extends Controller
{
   ...
   public function danhsachtaikhoan_admin()
    ...
    public function postcapnhattaikhoan_admin(CapnhatTaikhoanAdminRequest $reqcapnhattk)
    {
        ...
        {
            if((Auth::user()->id==$reqcapnhattk->ma && $trangthai==0))
            {
                ...
                ->update(
                    [
                        ...
                        'power' => $power,
                        'status' => $reqcapnhattk->input('status'),
                    ]
                );
    ...
    public function postCapnhatslide_admin(Request $reqcapnhat)
    {
         if($reqcapnhat->file('hinh_san_pham'))
        {
            if($reqcapnhat->file('hinh_san_pham')->isValid())
            {
                $reqcapnhat->file('hinh_san_pham')->move("source/images/slide",$reqcapnhat->file('hinh_san_pham')->getClientOriginalName());
                $hinh = $reqcapnhat->file('hinh_san_pham')->getClientOriginalName();
                //print_r($request->file('hinh_san_pham')->getClientOriginalName());exit;
            }
        }
        else
