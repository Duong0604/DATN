...
class AdminController extends Controller
{
    ...
    {  
        $thongtindonhang = DB::table('order')
            ...
            ->join('payment_status','order.payment_status','=','payment_status.id')
            ->join('order_status','order.status','=','order_status.id')
            ->selectRaw('order.id as id_order, order.status as status_order, order.date_order, order.total,order.id_promo, order.note as note_order,order.payments, order.payment_status,
            payment_status.id as id_payment, payment_status.name as name_payment, order_status.id as id_orderstatus, order_status.name as name_orderstatus, customer.*')
            ->where('order.id','=',$id)
            ->first();
        $chitiet = DB::table('order_detail')
            ...
            order_detail.id as id_orderdetail, order_detail.order_id,order_detail.product_id, order_detail.quantity, order_detail.unit_price,
        ...
        //     ->where('order_detail.order_id','=',$id)
        //     ->get();
         if($thongtindonhang->id_promo!=null)
        {
            $km = DB::table('promotions')
                ->join('order','order.id_promo','=','promotions.id')
                ->selectRaw('order.id_promo, promotions.promo_value, promotions.id_type_promo')
                ->where('order.id','=',$id)
                ->first();
             return view('admin.capnhatdonhang_admin',['thongtindonhang'=>$thongtindonhang,'chitiet'=>$chitiet,'dstrangthai'=>$dstrangthai,'km'=>$km]);
        }
        else
        {
    ...
    {
        $loaisp = Category::orderBy('id','desc')->paginate(10);
    ...
    {
        $sanpham = products::with('Category')->orderBy('id','desc')->paginate(10);
    ...
    public function postCapnhatsp_admin(ThemspadminRequest $reqcapnhat)
    {
        ...
        // );
            ...
            $product->new = $moi;
            $product->status = $reqcapnhat->status;
